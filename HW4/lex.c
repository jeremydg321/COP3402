...
void printOutput(FILE* input, FILE* output,tokStruct table[], int ind );
int ifInvisibleSpace(int m);
void printString(token_type  item);
...
while(charc!=EOF)
{
    ...
    /*fills String to size*/
     ...
     while(isalpha(charc=fgetc(input)) || isdigit(charc))
        ...
        /*prints out symbols*/
        printOutput(input2, output,lexer, ind);
...
/*Prints output*/
void printOutput(FILE* input,FILE* output, tokStruct lexer[], int ind)
{
